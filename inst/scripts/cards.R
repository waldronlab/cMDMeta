...
server <- function(input, output, session) {
    ...
    class_counts <- reactive({
        data <- filtered_data()
        all_classes <- unique(mpg$class)
        counts <- table(factor(data$class, levels = all_classes))
        data.frame(
            Class = names(counts),
    ...
    filtered_data <- reactive({
        ...
        if (length(selected) > 0) {
            data <- data[data$class %in% selected, ]
        }

        # Filter by cylinders if any are selected
        if (!is.null(input$selected_cyls) && length(input$selected_cyls) > 0) {
            data <- data[data$cyl %in% input$selected_cyls,]
        }

        # Filter by MPG range (using highway MPG)
        if (!is.null(input$mpg_range)) {
            data <- data[data$hwy >= input$mpg_range[1] & data$hwy <= input$mpg_range[2],]
        }

        # Return empty dataset if no data matches filters
        if (nrow(data) == 0) {
            return(data[0,])
        }
                        ...
                        box(
                            ...
                            div(class = "controls-container",
                                pickerInput(
                                    ...
                                    label = NULL,
                                    choices = sort(unique(mpg$cyl)),
                                    multiple = TRUE,
                                    selected = sort(unique(mpg$cyl)),
                                    options = list(`actions-box` = TRUE)
                                ),
                                div(
                        ...
                        box(
                            ...
                            div(class = "controls-container",
                                sliderInput(
                                    inputId = "mpg_range",
                                    label = NULL,
                                    min = floor(min(mpg$hwy)),
                                    max = ceiling(max(mpg$hwy)),
                                    value = c(floor(min(mpg$hwy)), ceiling(max(mpg$hwy))),
                                    step = 1
                                ),
                                div(
                                    style = "text-align: right;",
